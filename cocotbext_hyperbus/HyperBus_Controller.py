from cocotbext_hyperbus.HBC_FSM import HyperBus_FSM
from cocotb.triggers import Timer
from cocotb.binary import BinaryValue
import cocotb
from cocotb.handle import Release, Freeze, Force
from cocotb.utils import get_sim_time
from hyperbus_memory import HyperBusMemory  # Import the HyperBusMemory class
...
class HyperBusController(HyperBus_FSM):
    def __init__(self, dut, dq_driver, rwds_driver, cs_driver):
        ...
        self.dut = dut
        self.hyperbus_memory = HyperBusMemory(dut, dut.clk, dut.rst)  # Initialize HyperBusMemory
        self.Init(dut)
    ...
    async def ReadMem(self, addr, count):
        ...
        for i in range(count):
            ...
            self.i_mem_valid = 0
            await self.wait_until_mem_ready()
            data = await self.hyperbus_memory.read(r_addr)  # Read from HyperBusMemory
            r_data.append(data)
            self.log('Read operation complete.')
            self.log(f'Address: {hex(r_addr)}   Data: {hex(data)}')
            print("--------------------------------------------------")    
            await Timer(20, 'ns')
            r_addr += 2
        return r_data
